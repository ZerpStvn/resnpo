  ...
  <div class="as-container flex flex-row global-width">
    ...
    <div class="as-text-col flex-column flex">
      ...
      <div class="as-details flex-row">
        <?php
        function generateDetails($degree, $university, $subjects, $thesis, $isOpen = false)
        {
          ob_start();
        ?>
          <details <?php echo $isOpen ? 'open' : ''; ?>>
        ...
        function generateHonorsDetails($title, $honors, $isOpen = false)
        {
          ob_start();
        ?>
          <details <?php echo $isOpen ? 'open' : ''; ?>>
        ...
        <div class="detail-col-1">
          <?php
          echo generateDetails('博士号（PhD）', 'リバプール大学', '経営学/人事管理/情報学', 'ソフトウェア・アズ・ア・サービス（SAAS）モデル：候補者関係管理とキャリアパス計画への影響', true); // Open by default
        ...
        <div class="detail-col-2">
          ...
          echo generateHonorsDetails('栄誉/ 受賞', $honors, true); // Open by default
